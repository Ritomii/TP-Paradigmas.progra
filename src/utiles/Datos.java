...
public class Datos {
	...
	/*
	 * A partir de aca se cargan los arboles ordenados que se usaran para las no
	...
	public void ordenarNoPreferencias() {
		...
		String[] tipos_atraccion = this.mapa_atracciones_tipos.keySet()
				.toArray(new String[mapa_atracciones_tipos.size()]);

		// contiene todas las preferencias, los tipo_atraccion
		if (!this.mapa_promos_tipos.isEmpty())
			for (int i = 0; i < tipos_promos.length; i++) {
				this.mapa_no_preferencia_promociones.put(tipos_promos[i],
						this.obtenerArbolPromocionSinTipo(tipos_promos[i]));
			}

		for (int i = 0; i < tipos_atraccion.length; i++) {
			this.mapa_no_preferencia_atraccion.put(tipos_atraccion[i],
					this.obtenerArbolAtraccionSinTipo(tipos_atraccion[i]));
		}
	...
	private TreeSet<Promocion> calcularArbolTotalPromocion() {
		...
		for (Map.Entry<String, TreeSet<Promocion>> entry : this.mapa_promos_tipos.entrySet()) {
			// Viaja por cada Promocion del TreeSet
	...
	private TreeSet<Atraccion> calcularArbolTotalAtraccion() {
		...
		for (Map.Entry<String, TreeSet<Atraccion>> entry : this.mapa_atracciones_tipos.entrySet()) {
			// Viaja por cada Atraccion del TreeSet
